...
import com.yunsheng.xlock.task.AtomicLockFireEventTask;
import com.yunsheng.xlock.task.AtomicLockRemainTask;
import com.yunsheng.xlock.utils.XLockEventEnum;
import com.yunsheng.xlock.utils.XlockUtil;
...
public abstract class AtomicLockManager implements XLockManager<AtomicLock> {
    ...
    public void remainLock(AtomicLockRemainTask task) {
        ...
        if (this.enableLogWarn) {
            ...
            buf.append(" >> ").append(XlockUtil.nowDatetime()).append(" remainLock thread=").append(thread.getName())
    ...
    public void signalAllIfFree(AtomicLock atomicLock) {
        ...
        if (!isLocked) {
            ...
            if (this.enableLogWarn) {
                StringBuilder buf = new StringBuilder(120);
                buf.append(" >> ").append(XlockUtil.nowDatetime()).append(" AtomicLockCheckFreeTask ").append(" lock=")
                    .append(atomicLock
